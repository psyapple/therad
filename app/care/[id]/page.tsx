...
import { careServices, defaultCareProcess, getCareService, getRelatedGuidesForService } from "@/lib/content";
    ...
    <>
      ...
      <main className={`care-service-page care-service-${service.id}`}>
        <nav className="care-breadcrumb" aria-label="현재 위치">
          <div className="shell">
            <Link href="/">HOME</Link><span aria-hidden="true">/</span><Link href="/care">CARE</Link><span aria-hidden="true">/</span><strong>{service.title}</strong>
          </div>
        </nav>
        ...
        <section className="section care-approach-section">
          <div className="shell care-approach-grid">
            <div>
              <span className="section-kicker">AT SAEBYEOKBYEOL</span>
              <h2>새벽별에서는</h2>
            </div>
              ...
              {service.methods.length > 0 && (
                <div className="care-method-list">
                  {service.methods.map((method) => {
                    ...
                    return method.relatedGuide ? <Link href={`/guide/${method.relatedGuide}`} key={method.title}>{content}</Link> : <div key={method.title}>{content}</div>;
                  })}
                </div>
              )}
            </div>
        ...
        {relatedGuides.length > 0 && (
          <section className="section care-related-guides">
            <div className="shell">
        ...
        {relatedTools.length > 0 && (
          <section className="section care-related-tools">
            <div className="shell">
        ...
        <section className="care-service-cta">
          <div className="shell">
            ...
            <Link className="button button-light" href="/contact">상담 문의하기 <span>→</span></Link>
          </div>
        </section>
      </main>
      <Footer />
